...
$found = false;
$query = "SELECT * FROM Person where disabled=0 and uname = '" . $uname . "' and upassword= '" . $password . "'";
$result = executeQuery($query);
if($row = $result->fetchRow())
{
$found = true;
$expire=time()+60*60*24*30;
setcookie("personid",$row[0],$expire);
echo "<script language=javascript> document.cookie='username=" . $row[2] . "';document.cookie='usertype=" . $row[4] . "';</script>";
if($row[4] == "analyst") {
